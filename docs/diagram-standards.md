...
**Important**: We use color **sparingly and deliberately** to support accessibility and light/dark mode compatibility.
...
**Text color override** (when needed for contrast):
```
style NodeID fill:#333,stroke:#333,color:#fff
```

Use this only when a darker fill is essential for distinguishing node types.
...
We use **three architectural views** consistently throughout this repo. See [architectural-views.md](architectural-views.md) for detailed explanations.
...
### Simple Example (Module View)

See: `examples/00-hello-world-console/README.md`

```mermaid
graph LR
    hello[hello.js] -.->|runs in| node((Node.js))
```

### Medium Example (Component-Connector View)

See: `examples/02-lemonade-static-web/README.md`

```mermaid
flowchart LR
    User((User)) -->|clicks| Button[Button Element]
    Button -->|calls| Handler[Event Handler]
    Handler -->|updates| DOM[DOM]
    DOM -->|displays| Result[\Updated Page\]
```

### Complex Example (Allocation View)

See: `examples/04-lemonade-fullstack/README.md`
...
- [architectural-views.md](architectural-views.md) — When to use each view type
- [adr/0003-diagram-conventions.md](../adr/0003-diagram-conventions.md) — Why these conventions
...
These standards will evolve as the curriculum grows. Proposed changes should be documented in ADRs (Architecture Decision Records) in the `adr/` directory.

**Process**:
1. Identify a limitation or improvement
2. Draft an ADR (use `adr/template.md`)
3. Discuss in a GitHub issue or PR
4. Update this document once accepted
5. Update relevant diagrams to match new standards
