...
✅ Clear (specifies view):
```
"In the module view, which file contains the pricing logic?
...
✅ Clear (specifies view):
```
"Show me the component-connector view of what happens when a user
submits an order. Include validation, state updates, and UI feedback."
```
...
✅ Clear (specifies view):
```
"Create an allocation view diagram showing where each part of the
lemonade stand app should be deployed. Include browser, server,
database, and any CDN or static hosting."
```
...
## Teaching Progression in This Curriculum

### Part I: Introduce Views Gradually

**Module 01** (CLI app):
- Module View only (simple: one file)

**Module 03** (Static web):
- Add Component-Connector View (show data flow)

**Module 10** (Fullstack):
- Add Allocation View (client vs server)

### Part III: Use All Three Views

Every stage of the lemonade stand includes all three views:

- **Stage 1 (CLI)**: Simple examples
- **Stage 3 (SPA)**: Moderate complexity
- **Stage 4 (Fullstack)**: Full complexity

By the end, learners can:
- Draw all three views for any system
- Switch between views mentally
...
### Mistake 1: Confusing Views

❌ **Wrong**: Showing runtime data flow in a module view
✅ **Right**: Use component-connector view for runtime flow

❌ **Wrong**: Showing file structure in an allocation view
✅ **Right**: Use module view for file structure

### Mistake 2: Overcomplicating

**Don't** try to show everything in one diagram.

**Do** create multiple diagrams, each focused on one view.

### Mistake 3: Ignoring Views

**Don't** create random diagrams without a view in mind.

**Do** label each diagram with its view type.
...
**Module View**:
```
"In the module view, analyze dependencies. Are there any unnecessary
imports or large libraries that slow down the build?"
```

**Component-Connector View**:
```
"In the component-connector view, show the state update flow. Are there
unnecessary re-renders or inefficient state updates?"
```

**Allocation View**:
```
"In the allocation view, examine network calls. Are there ways to reduce
latency between browser and server?"
```
...
- **devfoundry docs**:
  - [diagram-standards.md](diagram-standards.md) — How to draw views
  - [prompts/02-architecture-first.md](../prompts/02-architecture-first.md) — Using views in prompts
...
- Review [diagram-standards.md](diagram-standards.md) to learn how to draw these views
- Explore [examples/](../examples/) to see all three views in practice
- Start [curriculum/part-1-foundations/01-what-software-is.md](../curriculum/part-1-foundations/01-what-software-is.md) to begin learning
