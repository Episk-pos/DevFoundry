...
- **When to use**: System installs/updates, like `sudo brew install git` or `sudo softwareupdate -i -a`.
- **Example**:
  ```bash
...
- **Risks/Warnings**:
  ...
  - Commonly used for Homebrew installs.
...
- **When to use**: Rare for devs; full root sessions (e.g., `su -` then run admin commands). Prefer `sudo`—it's audited and doesn't keep you as root.
- **Example**:
  ```bash
...
### sudo (Recommended: Safer and More Common)
- **What it does**: Prefixes a command to run it as root (e.g., `sudo ls /root`). Prompts for your user password (not root's).
- **When to use**: System installs/updates, like `sudo apt update` (Ubuntu/Debian) or `sudo dnf install git` (Fedora).
- **Example**:
  ```bash
  sudo npm install -g create-react-app  # Global install; enter password when prompted
  # But prefer: npx create-react-app my-app  # No elevation needed
...
- **Risks/Warnings**:
  - Never `sudo rm -rf /` (deletes entire system).
  - Avoid `sudo` in home directories—use `chown` if permissions are wrong.
  - For package managers like apt/yum/dnf.
- **Help**: `man sudo` or `sudo --help`.
