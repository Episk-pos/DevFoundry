...
> **Pick your OS** — Select your operating system in any tabbed section below. Your choice syncs across the entire page.
...
- **Command Prompt (CMD)** — legacy Windows terminal

</TabItem>
<TabItem value="macos" label="macOS">

- **Terminal.app** — built-in, found in Applications > Utilities
- **iTerm2** — popular third-party alternative

</TabItem>
<TabItem value="linux" label="Linux">

- **GNOME Terminal** — default on GNOME-based desktops (Ubuntu, Fedora)
- **Konsole** — default on KDE-based desktops
- **xterm** — lightweight, available everywhere
...
Use **Git Bash** (comes with [Git for Windows](https://git-scm.com/downloads)) — commands match macOS/Linux tutorials. PowerShell works but has different syntax.

</TabItem>
<TabItem value="macos" label="macOS">

Use the built-in **Terminal** app. Your shell is likely Zsh.

</TabItem>
<TabItem value="linux" label="Linux">

Use your distro's terminal. Your shell is likely Bash.
...
In **PowerShell**, the same command works via an alias, or you can use the native cmdlet:
```powershell
Get-Location
# Or the alias:
pwd
...
In **CMD**, use `dir` instead. In **PowerShell**, `ls` works as an alias for `Get-ChildItem`.
...
In **CMD**, use `cls` instead. In **PowerShell**, both `clear` and `cls` work.

**Keyboard shortcut**: `Ctrl + L` works in most terminals, including Git Bash.
...
Clears all the text from your terminal.
...
|--------|---------|---------|
| `C:\` | Root of C drive | `cd C:\` |
| `/` | Root (in Git Bash) | `cd /` |
| `~` | Home directory | `cd ~` |
| `.` | Current directory | `./run.sh` |
| `..` | Parent directory | `cd ..` |

In Git Bash, paths use forward slashes (`/c/Users/yourname`). In CMD and PowerShell, paths use backslashes (`C:\Users\yourname`).
...
Output (example):
```
/c/Users/yourname/bin:/mingw64/bin:/usr/local/bin:/usr/bin:/bin
```
...
That file *is* the Git program. When you type `git`, your shell finds and runs that file.
...
On Windows, the file extension (`.exe`, `.bat`, `.ps1`) determines if a file is executable.

In **Git Bash**, you can also use Unix-style execute permissions:
```bash
...
On macOS, a file needs "execute permission" to run:
...
On Linux, a file needs "execute permission" to run:
...
1. View your PATH:
   - In **Git Bash**: `echo $PATH`
   - In **PowerShell**: `$env:PATH`
2. Find where `node` is installed:
   - In **Git Bash**: `which node`
   - In **PowerShell**: `Get-Command node`
3. List what's in that directory

<details>
<summary>Solution (Git Bash)</summary>

```bash
echo $PATH
which node
# Outputs something like: /c/Program Files/nodejs/node
...
2. Find where `node` is installed: `which node`
3. List what's in that directory
...
Throughout this module, tabbed sections show Windows-specific instructions alongside macOS and Linux. Here's a summary of why we recommend Git Bash and how the alternatives compare.

### Why Git Bash?

[Git Bash](https://git-scm.com/downloads) comes bundled with Git for Windows and is what we recommend for this curriculum because:
...
PowerShell is pre-installed on all Windows systems. It's powerful, but has different syntax. Here's a quick reference:
...
WSL runs a full Linux environment inside Windows. More heavyweight than Git Bash, but useful if you need Linux-specific tools. For this curriculum, Git Bash is sufficient. Consider WSL later if you need full Linux capabilities.
